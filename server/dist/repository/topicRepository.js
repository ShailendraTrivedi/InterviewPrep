...
async function findById(id) {
    return Topic_1.Topic.findById(id).select('groupId categoryId name title').lean();
}
async function findByIds(ids) {
    if (ids.length === 0)
        return [];
    return Topic_1.Topic.find({ _id: { $in: ids.map((id) => new mongoose_1.default.Types.ObjectId(id)) } })
